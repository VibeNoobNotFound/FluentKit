...
Regenerate the consumer API reference and synchronize the skill bundle after changing the
library API:

```bash
./scripts/update-consumer-reference.sh
```

On Windows, run:

```bat
scripts\update-consumer-reference.bat
```
...
copies `api.json` into `fluentkit-consumer/references`, checks the summary baseline, and verifies
